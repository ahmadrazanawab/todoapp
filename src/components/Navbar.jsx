...
const Navbar = ({open,setOpen,showModalBtn,setQuarry}) => {
    const { allDeleteTodo } = useContext(noteProvider);
    
    const DeleteTodoAll = (e) => {
        e.preventDefault();
        allDeleteTodo();
        setOpen(!open);
    }
    ...
    <div className='shadow-md w-full fixed top-0 left-0'>
          <div className='md:flex items-center justify-between bg-[#f2f2f2] py-4 md:px-10 px-7'>
              ...
              <div className={`md:flex bg-[#ddd] md:bg-[#f2f2f2]  items-center  md:pb-0 pb-12 absolute md:static   md:z-auto z-[-1] left-0 w-full md:w-auto md:pl-0 px-2 my-2 transition-all duration-500 ease-in ${open ? 'top-16 opacity-100':'top-[-490px]'} md:opacity-100 opacity-0`}>
                  <form action="" className="flex items-center text-center md:flex-row flex-col justify-center " >
                      ...
                      <button onClick={DeleteTodoAll} className='flex md:mx-2 items-center bg-red-500 text-white px-4 md:px-6 py-2 rounded-md md:py-2  md:my-0 my-4  font-semibold'><span><RiDeleteBin5Line className="text-white size-5 " /></span><small>Delete All</small></button>
